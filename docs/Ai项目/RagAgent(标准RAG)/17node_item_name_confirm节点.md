---
title: 主体确认节点 (node_item_name_confirm)
date: 2026-07-03
abstract: LLM识别item_name,然后再通过milvus混合检索确认主体
tags:
- Ai实战项目
---
...
### 9.1 主体确认节点 (node_item_name_confirm)
...
请结合历史会话和当前用户问题，提取核心实体并改写问题：

1. **提取核心实体 (item_names)**：
- 识别用户当前询问的核心产品、设备型号、软硬件模块、技术实体或核心业务主题名称。
- ⚠️ **指代与确认消解**：如果当前问题是“对”、“是的”、“就是这个”、“第一个”等确认或选择动作，请从历史会话助手的最后一次回复中，提取出对应的实体名称填入列表中。
- 若无法确定核心实体，返回空列表 `[]`。

2. **问题改写 (rewritten_query) - 必须遵循以下规则**：
- **独立且完整**：改写后的问题必须是一个**无需任何上下文背景、无代词歧义**、可以直接用于搜索引擎或向量检索的独立完整句子。
- **消除指代**：将问题中所有的代词（如“这个”、“它”、“该系统”、“该机器”）替换为对应的具体产品或主题名称（如：“这个怎么操作” -> “BrotherHAK180烫金机怎么操作”）。
- **短句补全**：若当前提问过于简短（如“多少钱”、“怎么用”），必须将实体作为主语补全（如：“怎么用” -> “BrotherHAK180烫金机怎么使用”）。
- **确认场景改写**：若用户问题是“是的”、“对的”等确认词，改写后的问题应当还原为用户的**核心检索意图**（如：“是的” -> “我想了解BrotherHAK180烫金机的使用说明”）。

请直接返回 JSON 格式结果，不要包含任何 Markdown 代码块包裹（如 ```json），格式如下：
{{
"item_names": ["产品A"],
"rewritten_query": "关于产品A的..."
}}
```
