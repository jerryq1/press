...
--write， 触发提升磁盘写 IO 负载，会根据块大小的值来写入一个文件，比如块大小是
```
10，则固定的块的数量是 100，则会创建 1000M 的文件，销毁实验会自动删除
```
...
```
如果执行网络或者进程场景，无需指定，但这两个场景依赖 chaosblade-tool 镜像，默认是从 
```
[registry.cn-hangzhou.aliyuncs.com/chaosblade](http://registry.cn-hangzhou.aliyuncs.com/chaosblade) 仓库下载，也可以通过 --image-repo 参数指定，例如
