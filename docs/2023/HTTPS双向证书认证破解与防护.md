...
2. 被用于在Web浏览器和网站服务器之间传递信息，在TCP/IP中处于应用层。

这里提一下TCP/IP的分层共分为四层：应用层、传输层、网络层、数据链路层;
...
![](https://i.loli.net/2021/03/04/LREa2kDm6TbKG1t.png)
...
* 非对称加密： 加密和解密使用不同的密钥，这两个密钥形成有且仅有唯一的配对，叫公钥和私钥。数据用公钥加密后必须用私钥解密，数据用私钥加密后必须用公钥解密。一般来说私钥自己保留好，把公钥公开给别人（一般公钥不会单独出现，而是会写进证书中），让别人拿自己的公钥加密数据后发给自己，这样只有自己才能解密。 这种加密方式的特点是速度慢，CPU 开销大，常见非对称加密算法有 RSA，加密长度为*117byte*。
...
![](https://i.loli.net/2021/03/05/zx1L2dnqPBfDHAG.png)
...
> **秘钥F** 是三个随机数组合，分别在客户端发起首次请求、服务端首次回应返回证书、客户端校验证书完成发送加密随机数。根据这三个随机数，组合生成密钥F
> 
...
具体过程如下图所示：
![](https://i.loli.net/2021/03/05/dfAe6aKYxGr29hz.png)

#### 反抓包策略
为了防止中间人攻击，可以使用SSL-Pinning的技术来反抓包。
可以发现中间人攻击的要点的伪造了一个假的服务端证书给了客户端，客户端误以为真。解决思路就是，客户端也预置一份服务端的证书，比较一下就知道真假了。
SSL-pinning有两种方式： **证书锁定（Certificate Pinning）** 和 **公钥锁定（ Public Key Pinning）**。
...
为了防止预置证书被替换，在证书存储上，可以将证书进行加密后进行**「嵌入存储」**，如嵌入在图片中或一段语音中。这涉及到信息隐写的领域。
...
[使用Xposed+JustTrustMe来突破SSL Pinning](https://blog.csdn.net/hldh214/article/details/84310876)

[看如何逆向破解双向认证](https://blog.yii2.cc/mutual-tls-authentication/)

[如何突破双向证书认证防护](https://www.secpulse.com/archives/54027.html)